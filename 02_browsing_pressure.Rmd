...
load("processdata/browse_91.rda")
```
...
# m2lm <- lm(beitetrykkr ~ distvei3 + disthus + landsdel + skogkategori + kant + helling + treartgruppe, data = browse_91)
# 
# m3lm <- lm(beitetrykkr ~ distvei3 + disthus + skogkategori + kant + helling + treartgruppe, data = browse_91)

m4lm <- lm(beitetrykkr ~ distvei3 + disthus + region + skogkategori + kant + helling + hohkatb + treartgruppe, data = browse_91)
...
m2mix <- lmer(beitetrykkr ~ distvei3 + disthus + skogkategori + kant + helling + hohkatb + treartgruppe + (1|region), data = browse_91)

m3mix <- lmer(beitetrykkr ~ distvei3 + disthus + skogkategori + kant + helling + hohkatb + treartgruppe + (1|landsdel), data = browse_91)
...
AIC(m1lm, m2lm, m3lm, m4lm)
AIC(m1glm, m2glm, m3glm, m4glm, m5glm, m6glm, m7glm)
...
plot(predictorEffect("region", m4lm))
...
m1lmdist <- lm(beitetrykkr ~ disthus + landsdel + skogkategori + kant + helling + hohkatb + treartgruppe, data = browse91_dist)

```
```{r}
plot(allEffects(m1lmdist))
