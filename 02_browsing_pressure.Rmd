...
tglm <- glm(beitetrykk ~ distvei1 + disthus + landsdel + skogkategori + kant + helling + hohkatb, 
            family = quasipoisson, 
            data = browse_9)
tglm2 <- glm(beitetrykk ~ distvei1 + skogkategori + kant + helling + hohkatb + landsdel + treartgruppe, 
            family = quasipoisson, 
            data = browse_9)

# EP <- resid(tglm, type = "pearson")
# ED <- resid(tglm, type = "deviance")
# mu <- predict(tglm, type = "response")
# E <- browse_9$beitetrykk - mu
