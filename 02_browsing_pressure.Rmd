...
```{r}
gam13 <- gam(beitetrykkr ~ distvei1 + disthus+ skogkategori + treartgruppe + region + kant + helling + hohkatb + tretetthet, data = browse9bro_dist)

gam14 <- gam(bt100 ~ distvei1 + disthus+ skogkategori + treartgruppe + region + kant + helling + hohkatb + tretetthet, data = browse9bro_dist, method = "REML")
...
# zoib
starttime <- Sys.time()
m4zibr <- zoib(bt100 ~ distvei1 + disthus + skogkategori + treartgruppe + region + kant + helling + hohkatb + tretetthe|1|distvei1|1, 
            data = browse9bro_dist,
            zero.inflation = c(T), one.inflation = c(F))
endtime <- Sys.time()
endtime-starttime
save(m4zibr, file = "m4zibr.rda")
