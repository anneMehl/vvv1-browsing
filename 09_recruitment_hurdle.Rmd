...
family = binomial(),
data = recruits_house) # BEST; Same as rec_h_bin2, only run with disthus_600, so that both recruits house models have the same disthus variables - easier in the function
...
candmods_rec_house_bin <- list(rec_h_bin1, rec_h_bin2)
...
# 
#       K      AIC Delta_AIC AICWt Cum.Wt        LL
# Mod2 19 22777.47      0.00     1      1 -11369.73
# Mod1 19 22815.51     38.04     0      1 -11388.75
...
```{r}
library(sjPlot)
plot_model(rec_h_bin2_600, type = "diag")
plot_model(rec_h_bin2_600, type = "eff")
plot_model(rec_h_bin2_600, type = "est")
plot_model(rec_h_bin2_600_int, type = "int")
...
tab_model(rec_h_bin2_600, transform = NULL,
...
rec_h_beta3_TMB <- glmmTMB(recruitment ~ disthus_600 + log(distvei2+1) + skogkategori + treartgruppe9 + beitetrykk9 + kant + helling + HOH + I(HOH^2) +  ac + (1|KOMNR),
                           family = poisson,
                           data = recruits_house) # BEST; Same as rec_h_beta3, just run with glmmTMB
...
candmods_rec_house_beta <- list(rec_h_beta1, rec_h_beta2, rec_h_beta3)
...
# 
#       K     AIC Delta_AIC AICWt Cum.Wt        LL
# Mod3 18 1143746      0.00     1      1 -571855.0
# Mod2 18 1145904   2157.79     0      1 -572933.9
# Mod1 19 1145906   2159.75     0      1 -572933.9
...
plot_model(rec_h_beta3_TMB_int, type = "int")

# library(performance)
# # check_model(rec_h_beta7_TMB)
# model_performance(rec_h_beta3_TMB)

tab_model(rec_h_beta3_TMB, transform = NULL,
          show.intercept = TRUE, show.est = TRUE, show.ci = 0.95, p.threshold = c(0.05, 0.01, 0.001))
...
# Model with interaction for discussion
rec_r_bin3_int  <- glmer(recruitment_bin ~ log(distvei2+1)*beitetrykk9 + skogkategori + treartgruppe9 + kant + helling + HOH + I(HOH^2) + moose_density +  ac + (1|KOMNR),
family = binomial(),
data = recruits_roads)

rec_r_bin3_int2  <- glmer(recruitment_bin ~ distvei2*beitetrykk9 + skogkategori + treartgruppe9 + kant + helling + HOH + I(HOH^2) + moose_density +  ac + (1|KOMNR),
family = binomial(),
data = recruits_roads)

summary(rec_r_bin3_int)
summary(rec_r_bin3_int2)
...
plot_model(rec_r_bin3_int, type = "int")
plot_model(rec_r_bin3_int2, type = "int", mdrt.values = "quart")
...
# Model with interaction for discussion
rec_r_beta2_TMB_int <- glmmTMB(recruitment ~ log(distvei2+1)*beitetrykk9 + skogkategori + treartgruppe9 + kant + helling + HOH + I(HOH^2) + moose_density +  ac + (1|KOMNR),
family = poisson,
data = recruits_roads)

rec_r_beta2_TMB_int2 <- glmmTMB(recruitment ~ distvei2*beitetrykk9 + skogkategori + treartgruppe9 + kant + helling + HOH + I(HOH^2) + moose_density +  ac + (1|KOMNR),
family = poisson,
data = recruits_roads)

summary(rec_r_beta2_TMB_int)
summary(rec_r_beta2_TMB_int2)
...
plot_model(rec_r_beta2_TMB_int, type = "int")
plot_model(rec_r_beta2_TMB_int2, type = "int", mdrt.values = "quart")
# "minmax" -> (default) minimum and maximum values (lower and upper bounds) of the moderator are used to plot the interaction between independent variable and moderator(s).
# "meansd" -> uses the mean value of the moderator as well as one standard deviation below and above mean value to plot the effect of the moderator on the independent variable (following the convention suggested by Cohen and Cohen and popularized by Aiken and West (1991), i.e. using the mean, the value one standard deviation above, and the value one standard deviation below the mean as values of the moderator, see Grace-Martin K: 3 Tips to Make Interpreting Moderation Effects Easier).
# "quart" -> calculates and uses the quartiles (lower, median and upper) of the moderator value.
...
p_rec_bin1_house <- plot_model(rec_h_bin2_600, type = c("eff"), transform = NULL, terms = c("disthus_600"))
...
p_rec_beta1_house <- plot_model(rec_h_beta3_TMB, type = c("eff"), transform = NULL, terms = c("disthus_600"))
...
p_rec_beta8 <- p_rec_beta7 + theme_light() +
  theme(plot.title = element_blank(),
    axis.text = element_text(family = "sans", size = 20, color = "grey30"),
        axis.title = element_text(family = "sans", size = 20, color = "grey30")) +
  xlab("Moose density\n") +
...
library(gridExtra)
grid.arrange(p_rec_beta1_house, p_rec_beta1, p_rec_beta2, p_rec_beta3,
          nrow = 2, ncol = 2)

grid.arrange(p_rec_beta7, p_rec_beta4, p_rec_beta5, p_rec_beta6, p_rec_beta8,
          nrow = 3, ncol = 2)
