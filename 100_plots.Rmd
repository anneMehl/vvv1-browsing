...
uni_road <- unique(browse9.2$FLATEID[browse9.2$distvei2 <= "100"])
uni_road <- as.data.frame(uni_road) # 28

uni_house <- unique(browse9.2$FLATEID[browse9.2$disthus_cut400 <= "100"])
uni_house <- as.data.frame(uni_house) # 1


## Recruits
browse9.5msc$FLATEID <- as.factor(browse9.5msc$FLATEID)

uni_road <- unique(browse9.5msc$FLATEID[browse9.5msc$distvei2 <= "100"])
uni_road <- as.data.frame(uni_road) # 24

uni_house <- unique(browse9.5msc$FLATEID[browse9.5msc$disthus_400 <= "100"])
uni_house <- as.data.frame(uni_house) # alle
...
# recruits
# original
browse9.5msc$FLATEID <- droplevels(browse9.5msc$FLATEID)
browse9.5msc$treartgruppe9 <- droplevels(browse9.5msc$treartgruppe9)
browse9.5msc$FLATEID <- as.factor(browse9.5msc$FLATEID)
str(levels(browse9.5msc$FLATEID)) 
unique(browse9.5msc$FLATEID)
# reduced
browse9.6sc$FLATEID <- as.factor(browse9.6sc$FLATEID)
str(levels(browse9.6sc$FLATEID))


# browseing
# original
browse9.2$FLATEID <- as.factor(browse9.2$FLATEID)
str(levels(browse9.2$FLATEID))

# reduced
browse9.2_dist$FLATEID <- as.factor(browse9.2_dist$FLATEID)
str(levels(browse9.2_dist$FLATEID))
