...
recruits_test <- inner_join(recruits9, recruits10, by = "FLATEID", suffix =
c("_9", "_10"))
...
```{r}
# recruits_test is made i: #### Prepare data set for plant performance test

str(recruits_test)
table(as.factor(recruits_test$treartgruppe9))

recruits_test_rep <- recruits_test %>% slice(rep(1:n(), each = 3))
head(recruits_test_rep)

x <- factor(c("ROS","FURU","LAUV"))
recruits_test_rep$treartgruppe9 <- rep(x, 14479)
head(recruits_test_rep$treartgruppe9)
head(recruits_test_rep)
...
```{r}
recruits_test_rep$tretetthet9 <- ifelse(recruits_test_rep$treartgruppe9 == "ROS", recruits_test_rep$elgROSha9, NA)
recruits_test_rep$tretetthet9 <- ifelse(recruits_test_rep$treartgruppe9 == "FURU", recruits_test_rep$elgFURUha9, recruits_test_rep$tretetthet9)
recruits_test_rep$tretetthet9 <- ifelse(recruits_test_rep$treartgruppe9 == "LAUV", recruits_test_rep$elgLAUVha9, recruits_test_rep$tretetthet9)

head(recruits_test_rep$tretetthet9, n = 12)
sum(is.na(recruits_test_rep$tretetthet9)) #44, meaning that there were no trees to count? But then it should have been 0, no?
...
```{r}
recruits_test_rep$beitetrykk9 <- ifelse(recruits_test_rep$treartgruppe9 == "ROS", recruits_test_rep$elgbROSprs9, NA)
recruits_test_rep$beitetrykk9 <- ifelse(recruits_test_rep$treartgruppe9 == "FURU", recruits_test_rep$elgbFURUprs9, recruits_test_rep$beitetrykk9)
recruits_test_rep$beitetrykk9 <- ifelse(recruits_test_rep$treartgruppe9 == "LAUV", recruits_test_rep$elgbLAUVprs9, recruits_test_rep$beitetrykk9)

head(recruits_test_rep$beitetrykk9, n = 12)
sum(is.na(recruits_test_rep$beitetrykk9)) #21073, meaning that there could not have been browsing pressure, because there were no trees to browse on 


## NA the ones where tretetthet is 0, but we have beitetrykk values
recruits_test_rep$beitetrykk9 <- ifelse(recruits_test_rep$tretetthet9 != 0, recruits_test_rep$beitetrykk9, NA)
sum(is.na(recruits_test_rep$beitetrykk9)) #21182, meaning that there could not have been browsing pressure, because there were no trees to browse on  
...
```{r}
recruits_test_rep$proplt <- ifelse(recruits_test_rep$treartgruppe9 == "ROS", recruits_test_rep$ROS_25_49_PRHA, NA)
recruits_test_rep$proplt <- ifelse(recruits_test_rep$treartgruppe9 == "FURU", recruits_test_rep$Furu_25_49_PRHA_10, recruits_test_rep$proplt)
recruits_test_rep$proplt <- ifelse(recruits_test_rep$treartgruppe9 == "LAUV", recruits_test_rep$Lauv3, recruits_test_rep$proplt)

head(recruits_test_rep$proplt)
sum(is.na(recruits_test_rep$proplt)) #921, because there were no trees to count in that height class? Or where there no trees of the species? If there were trees of the species but not in height class, should in not be 0?
