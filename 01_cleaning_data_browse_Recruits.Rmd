...
sum(recruits$hogd10 == 1)+sum(recruits$hogd11 == 1) #1209
...
```{r}
library(readr)
moose_mean_density <- read_csv("processdata/five_y_mean_density.csv", na = "NA")
str(moose_mean_density)
library(tidyverse)
moose_mean_density <- moose_mean_density %>%
  rename(KOMNR = knr2017) %>%
...
```{r}
names(recruits)
recruits <- recruits[,-c(7:12,22:37)]
names(recruits)
```
...
nrow(recruits) 
nrow(na.omit(recruits)) # only NAs left are the ones in beitetrykk
...
```{r}
names(recruits)
recruits <- recruits[,-c(3:5,15,18)]
names(recruits)

save(recruits, file = "recruits.rda")

# Subset for roads data set
recruits_roads <- recruits[ which(recruits$disthus > 500), ] ## !!!!!!!!! ADJUST


```

#### Threshold for distance house, where houses do not have an influence anymore
```{r}
# House data set
load("processdata/recruits.rda")
recruits_house <- recruits

recruits_house$disthus_600 <- ifelse(recruits_house$disthus >= 600, 600, recruits_house$disthus)
recruits_house$disthus_500 <- ifelse(recruits_house$disthus >= 500, 500, recruits_house$disthus)
recruits_house$disthus_400 <- ifelse(recruits_house$disthus >= 400, 400, recruits_house$disthus)
recruits_house$disthus_350 <- ifelse(recruits_house$disthus >= 350, 350, recruits_house$disthus)
recruits_house$disthus_300 <- ifelse(recruits_house$disthus >= 300, 300, recruits_house$disthus)
recruits_house$disthus_200 <- ifelse(recruits_house$disthus >= 200, 200, recruits_house$disthus)
recruits_house$disthus_150 <- ifelse(recruits_house$disthus >= 150, 150, recruits_house$disthus)

str(recruits_house)

# recruits_house <- recruits_house[, -c(8)]
```
...
#### Save data set
```{r}
save(recruits_roads, file = "recruits_roads.rda")
