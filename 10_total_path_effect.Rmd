...
(focal = c("distvei", "skogkategori", "treartgruppe", "kant", 
           "helling", "HOH", "tretetthet", "browspres_merged", "disthus")[8])
...
recruit_kant <- resp_curve(focal, dat1, dat2, zeromod1, percmod1, zeromod2, percmod2, 
           which_effect=c("hurdle1_only", "hurdle2_only", "total")[2], nvals=100, plotit=F, nb_btstrp=99)

total_kant <- resp_curve(focal, dat1, dat2, zeromod1, percmod1, zeromod2, percmod2, 
...
mhusabin_2 <- glmer(factor(proplt > 0) ~ log(distvei2 + 1) + skogkategori +  
    treartgruppe9 + browspres_merged + kant + helling + HOH + I(HOH^2) + ac + (1 | KOMNR), 
    family = binomial(), data = browse9.5msc)

# beta
library(glmmTMB)
m16abeta

m16abeta_2 <- glmmTMB(proplt ~ log(distvei2+1) + skogkategori + treartgruppe9 + HOH + I(HOH^2) + browspres_merged + (1|KOMNR), data = browse9.5mpossc, family = beta_family())
...
m16cbin_2 <- glmer(factor(proplt > 0) ~ skogkategori + treartgruppe9 + kant + helling + HOH + I(HOH^2) +  ac + (1|KOMNR), family = binomial(), data = browse9.6sc)
...
m16abeta_2 <- glmmTMB(proplt ~ skogkategori + treartgruppe9 + HOH + I(HOH^2) + browspres_merged + (1|KOMNR), data = browse9.5mpossc, family = beta_family())

```

```{r}
library(AICcmodavg)
candmods <- list(m16cbin, m16cbin_2)

glmmbeta_tab <- aictab(candmods, modnames = NULL,
                       second.ord = TRUE, nobs = NULL, sort = TRUE)

library(bbmle)
ICtab(m16abeta, m16abeta_2, type = "AIC", weights = TRUE, delta = TRUE)
AIC(m16abeta, m16abeta_2)
