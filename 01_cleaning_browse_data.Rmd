...
browse9.5$treartgruppe <- browse9.5$treartgruppe9
...
ggplot(data = browse9.5m, aes(kant)) +
  geom_bar()+
  theme_classic()
...
ggplot(data = browse9.5m, aes(region)) +
  geom_bar()+
  theme_classic()
...
browse9.5m$treartgruppe9 <- factor(browse9.5m$treartgruppe9, levels = c("LAUV", "FURU"))
# browse9.2$hohkatb <- factor(browse9.2$hohkatb, levels = c("100", "0", "200", "300", "400", "500", "600", "700", "800"))
...
# subset on not ROS to avoid having double observations after creating "andtre"
browse9.6 <- browse9.5
browse9.6 <- browse9.5[browse9.5$treartgruppe9 != "ROS", ]

# Subset on different cuts for disthus
browse9.7 <- browse9.6[ which(browse9.6$disthus > 400), ]
save(browse9.7, file = "browse9.7.rda")
...
# Creating new variable(s) "andtre" (and "anttre")
...
browse9.5m <- subset(browse9.5m, select = -c(21,25:28))
...
# NaN in andtre, so again na.omit
# nrow(browse9.6)
# nrow(na.omit(browse9.6))
# 
# browse9.6 <- na.omit(browse9.6)
...
browse9.6$disthus_350 <- ifelse(browse9.6$disthus >= 350, 350, browse9.6$disthus)
