...
#### Model selection bin (browse9.5msc (house) and browse9.6sc (road))
```{r}
library(lme4)
load("processdata/browse9.5msc.rda")
...
ICtab(m0bbeta, m6beta, m7beta, m8beta, m10beta, m11beta, m12beta, type = "AIC", weights = TRUE, delta = TRUE)
AIC(m0bbeta, m6beta, m7beta, m8beta, m10beta, m11beta, m12beta)
...
##### Model selection (browse9.5mpos (house!!))
```{r}
library(glmmTMB)
load("processdata/browse9.5m.rda")
...
ICtab(m12beta, m13beta, m14beta, m15beta, m16abeta, type = "AIC", weights = TRUE, delta = TRUE)
AIC(m12beta, m13beta, m14beta, m15beta, m16abeta)
...
```{r}
library(lme4)
load("processdata/browse9.5m_region.rda")
pvars <- c("disthus_400",
           "helling","HOH", "browspres_merged", "ac")
...
# beta
m0bbeta_region <- glmmTMB(proplt ~ log(distvei2+1) + skogkategori + treartgruppe9 + ac + kant + helling + HOH + I(HOH^2) + browspres_merged + region + (1|KOMNR),
            data = browse9.5mpossc,
            family = beta_family()) 
...
load("processdata/browse9.6_region.rda")
load("processdata/browse9.6sc_region.rda")

pvars <- c("disthus_400","HOH","ac" ) #  "helling", "browspres_merged", "ac" 
browse9.6sc_region <- browse9.6_region
browse9.6sc_region[pvars] <- lapply(browse9.6sc_region[pvars],scale)

save(browse9.6sc_region, file = "browse9.6sc_region.rda")

# bin
m16cbin_region <- glmer(factor(proplt > 0) ~ log(distvei2+1) + skogkategori + treartgruppe9 + browspres_merged + helling + HOH + I(HOH^2) +  ac + region +  (1|KOMNR),
family = binomial(),
data = browse9.6sc)
