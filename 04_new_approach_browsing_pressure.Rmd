...
```
```{r}
m1glmm <- glmer(factor(beitetrykkr > 0) ~ disthus + treartgruppe + ac + kant + helling + HOH + I(HOH^2) + tretetthet + skogkategori*distvei1 + (1|KOMNR),
            family = binomial(link = "cloglog"),
            data = browse_9_dist)
```
...
- zeroinflated ist just one-way (meaning, no response, only for predictors), so I cannot use that for everything
- But, I can use it for the seconf part, to make a beta with random factor
```{r}
...
glmmhurdle <- glmmTMB(bt100 ~ disthus + treartgruppe + ac + KOMNR + kant + helling + HOH + I(HOH^2) + tretetthet + skogkategori*distvei1,
                      ziformula = ~.,
            family = beta_family(),
            data = browse_9_dist)
