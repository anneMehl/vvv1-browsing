...
coords <- as.matrix(cbind(browse_9$UTM_OV_33, browse_9$UTM_SN_33))

# compute the autocovariate based on the above distance and weight

ac <- autocov_dist(browse_9$bt100, coords, nbs = 5000, type="inverse", zero.policy = TRUE)
browse_9$ac <- ac
...
m1abin <- glm(factor(beitetrykkr > 0) ~ distvei1 + disthus + skogkategori + treartgruppe + ac + region + kant + helling + HOH + I(HOH^2) + tretetthet + skogkategori*distvei1,
            family = binomial(link = "cloglog"),
            data = browse_9)

browse_9$disthus[browse_9$disthus >= 300] <- 300

m1bbin <- glm(factor(beitetrykkr > 0) ~ distvei1 + disthus + skogkategori + treartgruppe + ac + region + kant + helling + HOH + I(HOH^2) + tretetthet + skogkategori*distvei1,
            family = binomial(link = "cloglog"),
            data = browse_9)
